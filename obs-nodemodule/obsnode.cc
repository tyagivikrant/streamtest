...
void Init(const FunctionCallbackInfo<Value>& args)
{
	...
	args.GetReturnValue().Set(String::NewFromUtf8(
		isolate, "!!! Node Native Module with OBS sucessfully loaded !!!").ToLocalChecked());
}

void ObsStart(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();

	bool retval = obs_startup("en-US", NULL, NULL);

	if (retval)
	{
		args.GetReturnValue().Set(String::NewFromUtf8(
			isolate, "!!! OBS started !!!").ToLocalChecked());
	}
	else
	{
		args.GetReturnValue().Set(String::NewFromUtf8(
			isolate, "!!! OBS startup failed !!!").ToLocalChecked());
	}
}

void ObsStream(const FunctionCallbackInfo<Value>& args)
{
	bool bStarted(false);
...
void ObsEnd(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	...
	args.GetReturnValue().Set(String::NewFromUtf8(
		isolate, "!!! OBS shutdown !!!").ToLocalChecked());
}

//void Initialize(Local<Object> exports)
//{
//	NODE_SET_METHOD(exports, "Init", Init);
//	NODE_SET_METHOD(exports, "ObsStart", ObsStart);
//	NODE_SET_METHOD(exports, "ObsStream", ObsStream);
//	NODE_SET_METHOD(exports, "ObsEnd", ObsEnd);
//}

//NODE_MODULE(NODE_GYP_MODULE_NAME, Initialize)

// creating context aware module
NODE_MODULE_INIT()
{
	...
	exports->Set(context,
		...
		FunctionTemplate::New(isolate, Init)->GetFunction(context).ToLocalChecked()).FromJust();

	exports->Set(context,
		String::NewFromUtf8(isolate, "ObsStart").ToLocalChecked(),
		FunctionTemplate::New(isolate, ObsStart)->GetFunction(context).ToLocalChecked()).FromJust();

	exports->Set(context,
		String::NewFromUtf8(isolate, "ObsStream").ToLocalChecked(),
		FunctionTemplate::New(isolate, ObsStream)->GetFunction(context).ToLocalChecked()).FromJust();

	exports->Set(context,
		String::NewFromUtf8(isolate, "ObsEnd").ToLocalChecked(),
		FunctionTemplate::New(isolate, ObsEnd)->GetFunction(context).ToLocalChecked()).FromJust();
}
